  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const totalRevenue = completedOrders.reduce((sum: any, order: any) => sum + (order.product?.price || 0), 0);

        setStats(prev => ({
          ...prev,
          totalSales: completedOrders.length,
          totalRevenue,
          pendingOrders: ordersData?.filter((order: any) => order.status === 'pending').length || 0
        }));
      } catch (error) {
